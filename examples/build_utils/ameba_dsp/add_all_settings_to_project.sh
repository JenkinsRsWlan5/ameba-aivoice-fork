...
:<<!
description:
Insert all aivoice example settings (dependencies, linked resources, etc) to a project_dsp
...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Error: project_dir and aivoice_example_name should be specified."
    echo "Usage: $0 <project_dir> <aivoice_example_name>"
    exit 1
fi
